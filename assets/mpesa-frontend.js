jQuery(document).ready(function($) {
    ...
    $('#mpesa-name-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            error: function() {
                alert('An error occurred during name verification. Please contact us at +254 727 054 097 for help.');
                $('#mpesa-name-modal').hide();
                resetForms();
            }
        });
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    switch (status) {
                        ...
                        case 'invalid_name':
                            clearInterval(statusCheckInterval);
                            showStatus('error', response.data || 'Payment failed due to invalid details. Please try again.');
                            resetPaymentForm();
                            break;
